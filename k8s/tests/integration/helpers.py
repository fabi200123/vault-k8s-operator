...
async def get_vault_client(
    ops_test: OpsTest, unit: Unit, token: str, ca_file_name: str | None = None
) -> Vault:
...
async def unseal_all_vault_units(
    ...
    vault = await get_vault_client(ops_test, leader, unseal_key, ca_file_name)
    if vault.is_sealed():
    ...
    for unit in app.units:
        assert isinstance(unit, Unit)
        vault = await get_vault_client(ops_test, unit, token, ca_file_name)
...
async def initialize_unseal_authorize_vault(ops_test: OpsTest, app_name: str):
    assert ops_test.model
